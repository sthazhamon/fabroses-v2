...
export async function onRequestPost({ request, env }) {
  ...
  const { customer_party_id, customer_name, reseller_name, customer_phone, delivery_address, order_date, promised_delivery_date, notes, items } = body;
  ...
  await env.DB.prepare(
    `INSERT INTO customer_orders (id, customer_party_id, customer_name, reseller_name, customer_phone, delivery_address, order_date, promised_delivery_date, status, notes)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'received', ?)`
  ).bind(id, customer_party_id || null, customer_name || null, reseller_name || null, customer_phone || null, delivery_address || null, order_date || new Date().toISOString().slice(0, 10), promised_delivery_date || null, notes || null).run();
