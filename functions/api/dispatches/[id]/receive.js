...
export async function onRequestPost({ request, env, params, data }) {
  ...
  const { confirmations, labor_cost, material_reconciliation } = body;
  ...
  const result = await confirmReceive(env, params.id, confirmations, data.user?.name, { labor_cost, material_reconciliation });
