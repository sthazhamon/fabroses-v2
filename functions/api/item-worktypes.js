...
export async function onRequestGet({ env }) {
  const { results } = await env.DB.prepare("SELECT * FROM item_work_types ORDER BY name ASC").all();
...
export async function onRequestPost({ request, env }) {
  ...
  try {
    const res = await env.DB.prepare("INSERT INTO item_work_types (name, code) VALUES (?, ?)").bind(name, code).run();
